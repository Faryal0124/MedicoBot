...
// Struct to represent one step in the diagnosis history
struct HistoryEntry {
    Node* node;//for backtracking last q k ans save ho jaye
    string answer;
};
...
public:
    ...
    void addDiagnosis( string& patientName,  string& summary) {
    ...
    void clearAllHistory(const string& filename) {
        ...
        // Overwrite file to clear its contents
    ...
    void printDiagnosesForPatient(const string& name) {
        ...
        if (count == 1) cout << "No records found for " << name << ".\n";
        cout << "=====================\n";
    }
    ...
    void saveToFile(const string& filename) {
        ...
        DiagnosisNode* temp = head;
        while (temp) {
            // Save patient name and summary, separated by delimiter "---ENTRY---"
    ...
    void loadFromFile(const string& filename) {
        ...
        while (getline(inFile, line)) {
            if (line == "---ENTRY---") {
                ...
                patientName = line;  // first line after previous entry is patient name
                readingSummary = true;
            }
...
// Stack to track user answers for backtracking
...
Node* buildSymptomTree() {
    ...
    return fever;  // This is the root node
}

// Ask questions and traverse tree; returns diagnosis summary string
string startDiagnosis(Node* root, const string& patientName) {
    answerHistory = stack<HistoryEntry>(); // Clear history each run
    Node* current = root;
    string input;
    ...
    cout << "(Tension nhi lo bhai nhi marte ap )\n";
...
int main() {
    ...
    cout << "=======  Welcome to the MedicoBot-Your health System Checker  =======\n\n";
    ...
    while (true) {
        ...
        int choice;
        cin >> choice;
        cin.ignore(); // to clear the leftover newline before next getline or input

        if (choice == 1) {
            string patientName;
            cout << "Enter patient name: ";
            getline(cin >> ws, patientName); // read patient name before diagnosis
            string result = startDiagnosis(root, patientName);
            history.addDiagnosis(patientName, result);

            // Save to file after adding new diagnosis
